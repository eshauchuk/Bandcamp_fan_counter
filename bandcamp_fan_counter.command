...
layout += [[sg.Button('Start selected',),
            sg.Button('Start all', bind_return_key=True)]]
...
def show_table(data, header_list, fn):
    ...
    window = sg.Window(fn, layout, grab_anywhere=True,
                       element_justification='c')

    # keep table open as a new window
    while True:
...
def analyze_n_display(track_list):
    ...
    header_list = ['track_fans', 'fans_combo', 'combo_tracks']
...
while True:
    ...
    if event in (sg.WINDOW_CLOSED, 'Exit'):
        ...
        track_list = [
            elem for elem in values_dict if values_dict[elem] == True]
        try:
            analyze_n_display(track_list)
        except Exception as e:
            print(e)
